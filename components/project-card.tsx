...
import { ArrowUpRight, Clock3, ReceiptText, Wallet } from 'lucide-react'

import { Badge, Card } from '@/components/ui'
import type { Project } from '@/lib/types'
import { formatCurrency, formatHours, getProjectPaymentStatus, paymentStatusClasses, paymentStatusLabel, statusLabel } from '@/lib/utils'
  ...
  return (
    <Card className="p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Badge className={paymentStatusClasses(paymentStatus)}>{paymentStatusLabel(paymentStatus)}</Badge>
          </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-2xl bg-slate-50 p-4">
          <div className="flex items-center gap-2 text-sm text-slate-500">
            <ReceiptText className="h-4 w-4" /> Tarif / oră
          </div>
          <p className="mt-2 text-2xl font-semibold text-slate-950">{formatCurrency(project.hourly_rate, project.currency)}</p>
        </div>
      </div>
    </Card>
  )
